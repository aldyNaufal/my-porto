...
import { FaArrowLeft, FaGithub, FaExternalLinkAlt, FaPlay, FaLock, FaFigma } from 'react-icons/fa';
import { WebMobileProjects } from './WebMobilesData';
import './WebMobiles.css';
...
function WebMobiles() {
  ...
  const filteredProjects = WebMobileProjects.filter(project => {
    if (activeStatus === 'all') return true;
    return project.status.toLowerCase() === activeStatus;
  });

  // Hitung jumlah untuk setiap status
  const completedCount = WebMobileProjects.filter(p => p.status.toLowerCase() === 'completed').length;
  const ongoingCount = WebMobileProjects.filter(p => p.status.toLowerCase() === 'ongoing').length;

  // Check if project is private (no links available or marked as private)
  const isPrivateProject = (project) => {
    return project.isPrivate || 
           (!project.githubLink && !project.liveLink && !project.playStoreLink && !project.figmaLink);
  };
    ...
    <div className="project-detail-page">
      <Container>
        <div className="project-detail-header">
          ...
          <h2 className="project-category-title">Web & Mobile Development Projects</h2>
                    ...
                    <div className="d-flex justify-content-between align-items-start mb-3">
                      <div className="flex-grow-1">
                        ...
                        <Card.Text className="text-muted mb-3">
                          <strong>Duration:</strong> {project.startDate} - {project.endDate} ({project.duration})
                        </Card.Text>
                      </div>
                ...
                {isPrivateProject(selectedProject) ? (
                  ...
                  <div className="d-flex justify-content-center flex-wrap gap-2">
                    ...
                    {selectedProject.figmaLink && (
                      ...
                      >
                        <FaFigma className="me-1" /> Figma Design
                      </a>
                    )}                    
                  </div>
                )}
