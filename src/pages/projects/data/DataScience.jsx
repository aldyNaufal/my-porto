...
import { SiStreamlit } from 'react-icons/si'; // Import icon Streamlit
import { DataScienceProjects } from './DataScienceData';
...
function DataScience() {
  ...
  const filteredProjects = DataScienceProjects.filter(project => {
    if (activeStatus === 'all') return true;
    return project.status.toLowerCase() === activeStatus;
  });

  // Hitung jumlah untuk setiap status
  const completedCount = DataScienceProjects.filter(p => p.status.toLowerCase() === 'completed').length;
  const ongoingCount = DataScienceProjects.filter(p => p.status.toLowerCase() === 'ongoing').length;

  // Check if project is private (no links available or marked as private)
  const isPrivateProject = (project) => {
    return project.isPrivate || 
           (!project.githubLink && !project.liveLink && !project.playStoreLink && !project.streamlitLink);
  };
    ...
    <div className="project-detail-page">
      <Container>
        <div className="project-detail-header">
          ...
          <h2 className="project-category-title">Data Science Projects</h2>
              ...
              <div className="project-links-section mt-4">
                {isPrivateProject(selectedProject) ? (
                  ...
                  <div className="d-flex justify-content-center flex-wrap gap-2">
                    ...
                    {selectedProject.streamlitLink && (
                      <a 
                        href={selectedProject.streamlitLink} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="btn btn-outline-danger btn-sm"
                      >
                        <SiStreamlit className="me-1" /> Streamlit App
                      </a>
                    )}
                  </div>
                )}
